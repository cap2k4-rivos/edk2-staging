  ...
  )
{
  ...
  mNsCommBuffMemRegion.PhysicalBase = MmVersionArgs.mmShmemAddrLow + EFI_PAGE_SIZE;
  // During boot , Virtual and Physical are same
  mNsCommBuffMemRegion.VirtualBase = mNsCommBuffMemRegion.PhysicalBase;
  mNsCommBuffMemRegion.Length      = MmVersionArgs.mmShmemSize - EFI_PAGE_SIZE;

  Addr           = 0;
  Addr           = Addr | (MmVersionArgs.mmShmemaddrHigh << 31 | MmVersionArgs.mmShmemAddrLow);
  MmCommonBuffer = (MM_COMM_BUFFER *)Addr;
  ...
  MmCommonBuffer->Status                  = Addr + sizeof (MM_COMM_BUFFER);
